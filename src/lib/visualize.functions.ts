...
import { getProduct, SLIM_BY_ID } from "@/lib/catalog";
 ...
 *
 * The WhatsApp webhook and the cron worker are raw routes with no TanStack
 * request context, so calling a server function from either throws "No Start
 * context found in AsyncLocalStorage". The logic lives in a normal function
 * that every caller shares, and the validation runs for all of them.
 */
export function parseVisualizeStart(input: StartInput) {
...
export type VisualizeStartData = ReturnType<typeof parseVisualizeStart>;

export async function runVisualizeStart(
  ...
  try {
    ...
    const products = data.productIds.map((id) => {
      // WhatsApp recommendations use the managed catalog: rendering must use
      // that same product record, not a stale bundled JSON counterpart.
      const product = channel === "whatsapp" ? getProduct(id) : catalog[id];
      ...
      const views = viewsFor(id).filter(
        (v) => channel !== "whatsapp" || product.images.includes(v.url),
      );
      const qty = data.quantities[id];
      return {
        ...product,
        col:
          (channel === "whatsapp" ? SLIM_BY_ID[id]?.col : slim.find((p) => p.id === id)?.col) ||
          null,
    ...
    const prompt =
      channel === "whatsapp" ? whatsappImagePrompt(request.prompt, data.mode) : request.prompt;

    const { uploadToKie, createVisualizeTask, resolutionFor, KIE_IMAGE_MODEL } =
      await import("@/lib/kie.server");
    ...
    const encoded = new TextEncoder().encode(
      ...
      // A model upgrade must invalidate earlier cached renders. Without this,
      // a repeat request could keep serving an image generated by the previous
      // model even though every new task uses the upgraded one.
 ...
 */
export function parseVisualizeStatus(input: { taskId: string }) {
  if (!input?.taskId || typeof input.taskId !== "string") throw new Error("taskId required");
  return { taskId: input.taskId };
}

export type VisualizeStatusData = ReturnType<typeof parseVisualizeStatus>;

export async function runVisualizeStatus(
  data: VisualizeStatusData,
